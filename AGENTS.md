...
- `vendor_researcher` 一次只负责主代理指定的一个厂商目录。它可以修改该目录内的报告、拆页、厂商概览页和芯片索引页；非中国厂商使用 `<vendor>-overview.md` / `<vendor>-chip-index.md`，中国厂商使用 `<中文厂商名>-概览.md` / `<中文厂商名>-芯片索引.md`，不得修改根级汇总或其他厂商目录。
...
- 同一厂商先更新综合报告，再同步拆页和索引；拆页重排证据，不独立升级综合报告没有支持的结论。
