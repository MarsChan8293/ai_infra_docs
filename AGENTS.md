...
- 芯片研究任务先动态扫描仓库一级厂商目录和其中的 Markdown 文件，任务清单以扫描结果为准，不使用硬编码厂商名单；系统架构整理任务按 `system/` 的 MOC 和现有概念节点确定边界。
...
本仓库同时是 GitHub Markdown 文档库和 Obsidian Vault。根入口为 [[00-ai-infra-map|AI Infra 知识图谱入口]]，系统入口为 [[system/README|AI Infra System Architecture]]，芯片入口为 [[chip/00-project-index|AI 芯片与基础设施资料库]]，模型入口为 [[models/00-model-index|AI Model Index]]。

- 仓库内部的语义关系优先使用 Obsidian Wiki Link，统一写成 `[[vault/root/path|显示名]]`。路径从仓库根开始，不写 `.md`。
- 外部官方资料、论文、博客和软件项目使用普通 URL，不要把外链伪装成 Wiki Link。
- 软件项目、社区、能力、集成和维护者关系的 canonical 事实位于 `ai_infra_relationship`；本仓库不建立第二份项目页，也不保留本地 redirect。
- `system/` 只建模跨项目稳定、且直接影响硬件选择或系统设计的机制，例如 KV Cache 内存层次、加速器资源模型、拓扑感知调度与异构推理。
- 使用 MOC 和概念节点表达跨目录关系。推荐主链路是 **Model → System → Chip**。
- 不要求每条关系都手写 A→B 和 B→A 两份链接。Obsidian Backlinks 已能显示反向边。
- 每个重要文档都应至少存在一条从 MOC、概念页或上级总览可达的入边，避免孤岛节点。
- 芯片与模型的跨域关系优先通过系统概念节点连接，避免在每个芯片页机械链接大量软件项目。
- 新增或重命名文档时，检查 Wiki Link 的目标是否唯一。移动文件时同步更新显式路径链接。
- 页面标题尽量唯一；需要多个常用名称时使用 YAML frontmatter 的 `aliases`。
- 不提交个人 Obsidian UI 状态，例如 `.obsidian/workspace.json`、`.obsidian/workspace-mobile.json`。
- 整理任务完成后除常规 Markdown 检查外，还要检查 unresolved Wiki Links、重复 title/alias、明显孤立节点，并在 Graph View 中抽查关系合理性。

推荐的系统架构骨架：[[system/memory/kv-cache-memory-hierarchy|KV Cache 内存层次]]、[[system/resource/accelerator-resource-model|加速器资源模型]]、[[system/topology/topology-aware-scheduling|拓扑感知调度]]、[[system/heterogeneous-compute/heterogeneous-inference|异构推理]]。
